...
func IncludedTestCases() []TestCase {
	...
	return []TestCase{
		...
		{
			"Start simple positive",
			nil,
			&pb.Pool{

				CreationTimeRangeFilter: &pb.Pool_CreationTimeRangeFilter{
					Start: timestamp(now.Add(time.Hour * 1)),
				},
			},
		},
		{
		...
		{
			"Between Start and End positive",
			nil,
			&pb.Pool{
				CreationTimeRangeFilter: &pb.Pool_CreationTimeRangeFilter{
					Start: timestamp(now.Add(time.Hour * 1)),
					End:   timestamp(now.Add(time.Hour * -1)),
				},
			},
		},
		{
...
func ExcludedTestCases() []TestCase {
	...
	return []TestCase{
		...
		},
		{
			...
			&pb.Pool{
				CreationTimeRangeFilter: &pb.Pool_CreationTimeRangeFilter{
					Start: timestamp(now.Add(time.Hour * 2)),
					End:   timestamp(now.Add(time.Hour * 1)),
				},
			},
		},
		{
			...
			&pb.Pool{
				CreationTimeRangeFilter: &pb.Pool_CreationTimeRangeFilter{
					Start: timestamp(now.Add(time.Hour * -1)),
					End:   timestamp(now.Add(time.Hour * -2)),
				},
			},
		},
...
func multipleFilters(doubleRange, stringEquals, tagPresent, creationTimeRange bool) TestCase {
	...
	return TestCase{
		fmt.Sprintf("multiplefilters: %v, %v, %v, %v", doubleRange, stringEquals, tagPresent, creationTimeRange),
		&pb.Ticket{
			Attributes: &pb.Ticket_FilterableData{
				DoubleArgs: map[string]float64{
					"a": a,

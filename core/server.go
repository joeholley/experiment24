...
func start(cfg *viper.Viper) {
	...
	logger.Debugf("Open Match Core gRPC server started")
	...
	go func() {
		if err = http.ListenAndServe(":58081", mux); err != nil {
			logger.Fatal(err)
		}
	}()
...
func (st *Handler) HandleRPC(ctx context.Context, stat stats.RPCStats) {
	...
	// HandleRPC is called multiple times during a single request-reponse cycle,
	...
	case *stats.End:
		...
		// the RPC context was created), retreive it here so it can be added as an
		...
		// Track rpc errors
		if s.Error != nil {
			RpcErrors.Add(ctx, 1, rpcNameAttr, metric.WithAttributes(attribute.String("error", s.Error.Error())))
		}
	}
}
